...
REM FOR %%a IN ("%serverMods:;=" "%") DO (
	REM IF %%a NEQ "" (
		REM CALL :c 06 "Loading Server Mod	"
		REM IF EXIST "%%~a" (
			REM CALL :c 0a "%%~a" /n
			REM SET "armaMods=!armaMods!%%~a;"
		REM ) ELSE (
			REM CALL :c 04 "%%~a" /n
		REM )
	REM )
...
IF %ERRORLEVEL% == 1 (
	REM CALL :c 0E "Starting "
	REM CALL :c 0A %launchID%
	REM CALL :c 0E " Server" /n

	REM start /min "%LAUNCHID%" %PROCESSTYPE% "%ARMAPATH%arma3server.exe" -port=%PORT% "-config=%CONFIGDATA%%LAUNCHID%_server.cfg" "-cfg=%CONFIGDATA%%LAUNCHID%_basic.cfg" "-profiles=%PROFILEDATA%" -name=%LAUNCHID% -filePatching "-mod=%ARMAMODS%" -autoInit -enableHT
	...
	start /min "%LAUNCHID%" %PROCESSTYPE% "%ARMAPATH%arma3server.exe" -client -connect=66.85.14.216 -port=%PORT% -password=%HEADLESSCONNECT% "-profiles=%PROFILEDATA%" -name=%LAUNCHID%-HC1 "-mod=%HCMODS%"
	start /min "%LAUNCHID%" %PROCESSTYPE% "%ARMAPATH%arma3server.exe" -client -connect=66.85.14.216 -port=%PORT% -password=%HEADLESSCONNECT% "-profiles=%PROFILEDATA%" -name=%LAUNCHID%-HC2 "-mod=%HCMODS%"
