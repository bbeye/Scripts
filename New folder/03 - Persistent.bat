...
SET launchID=Persistence


REM =========  Where is ArmA installed?  ==========
SET armaPath=C:\arma3\servers\persistence\
...
SET profileData=C:\arma3\profiles\persistence\


REM ===========  Where are my configs?  ===========
SET configData=C:\arma3\configs\persistence\


REM =============  Where is BattlEye?  ============
SET bePath=C:\arma3\servers\persistence\BattlEye\
...
IF %ERRORLEVEL% == 1 (
	...
	start "%LAUNCHID%" %PROCESSTYPE% "%ARMAPATH%arma3server.exe" -port=%PORT% "-config=%CONFIGDATA%%LAUNCHID%_server.cfg" "-cfg=%CONFIGDATA%%LAUNCHID%_basic.cfg" -profiles=%PROFILEDATA% -name=%LAUNCHID% -filePatching -autoInit -enableHT "-mod=%ARMAMODS%"

	timeout 20
	
	echo:
	FOR /L %%i IN (1,1,%headlessCount%) DO (
		
		CALL :c 0E "Starting "
		CALL :c 0A %launchID%
			CALL :c 0E " Headless Client #%%i" /n
		
		start "%LAUNCHID%" %PROCESSTYPE% "%ARMAPATH%arma3server.exe" -client -connect=127.0.0.1 -port=%PORT% -password=%HEADLESSCONNECT% -profiles=%PROFILEDATA% -name=HC%%i "-mod=%HCMODS%"
