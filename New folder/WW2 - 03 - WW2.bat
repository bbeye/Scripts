...
SET armaPath=C:\arma3\servers\WW2\
...
SET profileData=C:\arma3\profiles\WW2\


REM ===========  Where are my configs?  ===========
SET configData=C:\arma3\configs\WW2\


REM =============  Where is BattlEye?  ============
SET bePath=C:\arma3\servers\insurgency\WW2\
...
FOR /F "usebackq tokens=1* delims==>" %%I IN ("\arma3\configs\WW2_modlist.txt") DO (
    SET "modlist[%%~I]=%%~J"
)
...
IF %ERRORLEVEL% == 1 (
	...
	start /min "%LAUNCHID%" %PROCESSTYPE% "%ARMAPATH%arma3server.exe" -port=%PORT% "-config=%CONFIGDATA%%LAUNCHID%_server.cfg" "-cfg=%CONFIGDATA%%LAUNCHID%_basic.cfg" "-profiles=%PROFILEDATA%" -name=%LAUNCHID% -filePatching "-mod=%ARMAMODS%" -autoInit -enableHT
